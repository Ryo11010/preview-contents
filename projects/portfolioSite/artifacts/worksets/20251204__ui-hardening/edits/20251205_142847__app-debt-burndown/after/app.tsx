...
const PROJECTS: Project[] = [
  ...
  },
  {
    ...
    longDescription: 'レイヤー機能、アニメーション作成機能、カスタムパレット作成機能を備えた本格的なドット絵エディタ。Apple Pencilにも対応。',
...
const FolderCardItem = ({
  ...
  const getTransform = () => {
    if (isOpen) {
      // Fan out logic adapted for multiple cards
      ...
      let rotate = relativeIndex * (maxRotate / centerOffset);
      
      // Calculate Y (Arch effect)
      const yBase = -160;
      const archIntensity = 8;
      const y = yBase - (Math.abs(relativeIndex) * archIntensity); 

      // Alignment correction: Increase shift amount dynamically based on card count
      const shiftAmount = Math.min(Math.max(40, totalCards * 15), 160); // Min 40px, Max 160px based on card count
      
      if (align === 'left') {
        x += shiftAmount; 
        rotate += 5; // Slight rotation correction
      } else if (align === 'right') {
        x -= shiftAmount;
        rotate -= 5;
      }
  ...
  return (
    <div className="flex flex-col items-center">
        ...
        >
            {/* Back Plate */}
            <div
            className="absolute bottom-4 w-60 h-44 bg-[#75BCE6] rounded-3xl z-0 transition-transform duration-300 ease-out"
            style={{
    ...
    <div id="work" className="max-w-7xl mx-auto px-6 py-24 min-h-screen flex flex-col justify-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 justify-items-center">
        {categories.map((category, index) => {
            // Determine alignment based on column index (assuming 3 columns layout)
            ...
            <div className="flex flex-wrap justify-center gap-2 animate-in fade-in slide-in-from-bottom-2 duration-300">
              {profile.skills.map((skill) => (
                <span
                  key={skill}
                  /* デザイン修正:
                     - px-3 py-1.5 -> px-3 py-1: 高さをさらに減らす
                     - text-sm -> text-xs: 文字を小さく
                     - hover効果削除: 静的な表示に変更
                     - 色味: 少し落ち着いた色に変更
                  */
    ...
    <div className="pb-12">
      ...
      <div className="space-y-10">
        ...
        <div className="flex flex-col gap-3 pt-4">
          ...
          <div className="flex gap-3">
            {project.webUrl && (
              <LiquidButton size="lg" variant="glass" className="flex-1 bg-white border-slate-200" onClick={() => openExternalLink(project.webUrl!)} icon={Globe}>
                Website
              </LiquidButton>
            )}
            {project.githubUrl && (
               <LiquidButton size="lg" variant="glass" className="flex-1 bg-white border-slate-200" onClick={() => openExternalLink(project.githubUrl!)} icon={Github}>
                 Source Code
               </LiquidButton>
            )}
          </div>
