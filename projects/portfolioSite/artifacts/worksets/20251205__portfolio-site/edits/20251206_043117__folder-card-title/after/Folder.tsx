...
const FolderCardContent: React.FC<{ project?: Project }> = ({ project }) => {
  ...
  return (
    <div className="w-full h-full p-4 flex flex-col relative z-10">
      ...
      <h4 className="text-base font-bold text-white mb-1.5 leading-tight line-clamp-2">{project.title}</h4>
...
const FolderCardItem = ({
  ...
  return (
    <div
      className={`absolute bottom-0 left-0 right-0 mx-auto w-52 h-32 rounded-2xl border border-white/20 shadow-2xl overflow-hidden origin-bottom transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${bgGradient}`}
      style={{
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/55 via-transparent to-white/15 pointer-events-none" />
    </div>
  );
};
...
export const FolderComponent = ({
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      >
      <div
        className="absolute bottom-4 w-64 h-48 bg-[#75BCE6] rounded-3xl z-0 transition-transform duration-300 ease-out"
        style={{
      ...
      >
        ...
        <div
          className="relative z-40 w-64 h-[150px] bg-white/10 backdrop-blur-md rounded-[24px] border border-white/20 transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)]"
          style={{
